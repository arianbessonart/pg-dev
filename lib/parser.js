...
function parseFunction(fileContent) {
  ...
  if (matches !== null) {
    ...
    functionData = {
      ...
      signature: createSignature(matches[2], matches[3], matches[4], matches[5])
    };
  }
  return functionData;
}

function createSignature(schema, functionName, params, returnType) {
  var signature = returnType.trim() + ':' + schema.trim() + '.' + functionName.trim() + params.trim();
  return signature;
}
