...
function generateSelect(editor, bufferPosition) {
  var line = fCommon.getEntireLine(editor, bufferPosition);
  console.log(line);
  var currentWord = 'md_bt_get_by_id';//parser.getEntireWordByPosition(line, bufferPosition.column);
  ...
  if (functions[currentWord] !== undefined && functions[currentWord].length > 0) {
    ...
    generateSelectSql = 'SELECT ' + outParams + ' FROM ';
    console.log(generateSelectSql);
    textRange = [[bufferPosition.row, 0], [bufferPosition.row, bufferPosition.column]];
    atom.workspace.getActiveTextEditor().setTextInBufferRange(textRange, generateSelectSql);
  }
}
