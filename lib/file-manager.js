...
function loadCurrentFile(fileContent) {
  ...
  if (match !== null) {
    for (var i = 0; i < match.length; i++) {
      var varName = match[i].match(/\w+/g)[0];
      if (_.indexOf(dataDb.keywords, varName.toLowerCase()) === -1) {
        usedVariables[varName] = {name: varName};
      }
    }
  ...
  var regex = /insert\s+into\s+(\w+)\s*\(/ig;
  var match = bodyContent.match(regex);
  if (match !== null) {
    for (var i = 0; i < match.length; i++) {
      var kwAfterInsert = match[i].match(/insert\s+into\s+(\w+)\s*\(/i)[1];
      if (kwAfterInsert in usedFunctions) {
        delete usedFunctions[kwAfterInsert];
      }
    }
  }
