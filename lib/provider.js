...
function suggestDataTypes(prefix) {
  ...
  if (prefix !== undefined) {
    ...
    _.forEach(matches, function (value) {
      suggestions.push({"text": value, "type": "type", "description": dataDb.dataTypes[value].description, "rightLabel": dataDb.dataTypes[value].group});
    });
  } else {
    _.forEach(dataDb.dataTypes, function (value, key) {
      suggestions.push({"text": key, "type": "type", "description": value.description, "rightLabel": value.group});
    });
  }
  return suggestions;
}
...
function getSuggestions({editor, bufferPosition, scopeDescriptor, prefix, activatedManually}) {
  ...
  // var currentWord = parser.getWordByPosition(line, bufferPosition.column);
